...
class DiceLoss(_WeightedLoss):
    ...
    def _dice_loss_multichannel(output, target, weights=None, ignore_index=None):
        ...
        if ignore_index is not None:
            mask = target == ignore_index #mask=bool
            target = target.clone()
            target[mask] = 0
            # use 1 in dim1 direction to fill the encoded_target
            # utilize index in target_unsqueeze(1)
        ...
        numerator = 2 * intersection.sum(0).sum(1).sum(1)  # (139,)sum(0):139,256,256;sum(0).sum(1):139,256;sum(0).sum(1).sum(1) 139,
...
class CombinedLoss(_Loss):
    ...
    def forward(self, inputs, target,skull_gt,weight=None):
        """
        Forward pass
        ...
        :return: scalar
        """
        pred_fuse , pred_label, pred_skull= inputs
        ...
        if weight is None:
            ...
            y_1fuse = torch.mean(
                torch.mul(self.cross_entropy_loss.forward(pred_fuse, target), weight.cuda())) #mul .* of matrix
            y_1label = torch.mean(
