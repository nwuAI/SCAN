...
class TopkRouting(nn.Module):
    ...
    def forward(self, query:Tensor, key:Tensor)->Tuple[Tensor]:
        ...
        if not self.diff_routing:
            query, key = query.detach(), key.detach() #.detach() make tensor -> requires_grad=false
        query_hat, key_hat = self.emb(query), self.emb(key) # per-window pooling -> (n, p^2, c)  #identity
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, n_win=7, qk_dim=None, qk_scale=None,
        ...
        self.qk_dim = qk_dim or dim # qk_dim=dim
        assert self.qk_dim % num_heads == 0 and self.dim % num_heads==0, 'qk_dim and dim must be divisible by num_heads!'
        self.scale = qk_scale or self.qk_dim ** -0.5 #B_s_qkscale=default=none
        ...
        self.kv_downsample_mode = kv_downsample_mode #identity
        self.kv_per_win = kv_per_win #-1
        self.kv_downsample_ratio = kv_downsample_ratio#dont pass
        self.kv_downsample_kenel = kv_downsample_kernel#dont pass
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        attn_weight = self.attn_act(attn_weight) #softmax
