    ...
    def forward(self, x):
        ...
        for pool_type in self.pool_types:
            ...
            if channel_att_sum is None:
                ...
                channel_att_sum = channel_att_sum + channel_att_raw #4 64

        # scalecoe = F.sigmoid(channel_att_sum)
        channel_att_sum = channel_att_sum.reshape(channel_att_sum.shape[0], 8, 8) #4 8 8
        avg_weight = torch.mean(channel_att_sum, dim=2).unsqueeze(2) #4 8 1
        avg_weight = avg_weight.expand(channel_att_sum.shape[0], 8, 8).reshape(channel_att_sum.shape[0], 64) #4 64
        scale = torch.sigmoid(avg_weight).unsqueeze(2).unsqueeze(3).expand_as(x) # 4 64 256 256
...
class SpatialAtten(nn.Module):
    ...
    def forward(self, x):
        residual = x
        x_out = self.conv1(x)# 4 16 256 256
        x_out = self.conv2(x_out) # 4 16 256 256
        spatial_att = torch.sigmoid(x_out).unsqueeze(4).permute(0, 1, 4, 2, 3)
        spatial_att = spatial_att.expand(spatial_att.shape[0], 16, 4, spatial_att.shape[3],
                                         spatial_att.shape[4]).reshape(
            spatial_att.shape[0], 64, spatial_att.shape[3], spatial_att.shape[4])
